...
for( int index = 0; index < nodeCount; index ++ )
	graph[index] = new int[nodeCount];

bool visited = new bool[nodeCount];

void dfsGivenStart( int **graph, bool visited[], int nodeIndex )
{
	...
	{
		if( graph[nodeIndex][index] == 1 )
			dfsGivenStart( graph, visited, index );
	}
	return;
}

void dfs( int **graph, bool visited[] )
{
	//
	//
